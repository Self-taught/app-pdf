...
for filepath in filepaths:
    ...
    for index, row in df.iterrows():
        ...
        pdf.cell(w=35, h=8, txt=str(row["amount_purchased"]), align="C", border=1)
    ...
    # Total Sum
    pdf.set_font(family="Times", size=10, style="B")
    pdf.cell(w=30, h=8, txt=f"The total price is {total_sum}", ln=2)

    # Company Image
    pdf.set_font(family="Times", size=10, style="B")
    pdf.cell(w=30, h=8, txt=f"Go Enjoy", align="C")
    pdf.image("sloth.png", w=30)

    pdf.output(f"PDFs/output{filename}.pdf")
